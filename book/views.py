...
from .forms import AuthorCommentForm, BookCommentForm
from .models import Author, Book, BookComment, Contact
...
class BookDetail(SuccessMessageMixin, DetailView):
    ...
    success_message = (
        "دیدگاه شما با موفقیت ثبت شد و پس از تایید، در سایت به نمایش گذاشته خواهد شد."
    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(
                self.request,
                f"دیدگاه شما با موفقیت ثبت شد و پس از تایید، در سایت به نمایش گذاشته خواهد شد.",
            )
...
class ContactUs(SuccessMessageMixin, CreateView):
    ...
    success_message = (
        "پیغام شما با موفقیت ارسال شد. همکاران ما در اسرع وقت با شما تماس خواهند گرفت."
    )
...
class Search(ListView):
    ...
    def get_queryset(self):
        ...
        return Book.objects.filter(
            Q(status="A")
            & (Q(author__name__icontains=search) | Q(name__icontains=search))
        )
